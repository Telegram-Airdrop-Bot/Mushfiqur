...
export const DynamicVisitorAnalytics = () => {
  ...
  const generateSampleData = () => {
    ...
    // More realistic country distribution based on common markets
    const countries = [
      'Bangladesh', 'Bangladesh', 'Bangladesh', 'Bangladesh', 'Bangladesh', // 50% Bangladesh
      'United States', 'United States', 'United States', // 30% US
      'United Kingdom', 'United Kingdom', // 20% UK
      'Canada', 'Australia', 'India', 'Germany', 'France', 'Japan'
    ];
    
    // Cities corresponding to countries
    const cityMap = {
    ...
    // Generate sessions for the last 30 days with more realistic patterns
    for (let i = 0; i < 50; i++) {
      const sessionDate = new Date(now.getTime() - Math.random() * 30 * 24 * 60 * 60 * 1000);
      const deviceType = deviceTypes[Math.floor(Math.random() * deviceTypes.length)];
      const country = countries[Math.floor(Math.random() * countries.length)];
      const cities = cityMap[country as keyof typeof cityMap] || ['Unknown'];
      const city = cities[Math.floor(Math.random() * cities.length)];
      
      // More realistic chat history (most users have 0-3 messages, few have many)
      let chatMessages = 0;
      if (Math.random() > 0.7) { // 30% chance of having messages
        chatMessages = Math.floor(Math.random() * 8) + 1; // 1-8 messages
      }
      ...
      sampleSessions.push({
        ...
        chatHistory: Array.from({ length: chatMessages }, () => ({})),
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-2xl font-bold">Dynamic Visitor Analytics</h2>
